...
.setThumbnail()
.addField(`Cool Medya | Teşekkürler`, `**Selamlar, Ben Cool Medya (Cool Medya'nin Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Alone | Prefix`, `**Cool Medya Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`Cool Medya | Nasıl Kullanılır?`, `**Cool Medya botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`Cool Medya | Linkler`, `**Sohbet Kanalına -davet Yazmanız Yeterlidir**`)
.setFooter(`Cool Medya | Gelişmiş Türkçe Bot | 2020`)
.setTimestamp();
...
.setDescription(`<:a_:821738957997211659>  <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Cool Medya Bot | Log Sistemi")
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setFooter(`Cool Medya Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
