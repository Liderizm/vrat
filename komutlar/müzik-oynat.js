...
const sarki = await client.player.addToQueue(message.guild.id, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("Cool Medya | Kuyruğa Ekle")
...
const sarki = await client.player.play(message.member.voice.channel, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("Cool Medya | Oynatılan Şarkı")
...
sarki.queue.on('end', () => {
  ...
  .setColor(AloneDogru)
  .setTitle("Cool Medya | Kuyruk Bitti!")
  .setDescription(`<:a_:821738957997211659>  **| Kuyruktaki tüm müzikler oynatıldı. Cool Medya kanaldan ayrılıyor.\n\nCool Medya Tercih ettiğiniz için teşekkür ederiz.**`)
  .setThumbnail(client.user.avatarURL())
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());  
